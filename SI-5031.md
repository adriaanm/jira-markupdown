...
See also SI-5422 (marked as duplicate), which contains a program that compiled with 2.9.1 that no longer compiles.
...
I just ran into this as well when porting my code from 2.9.2 to 2.10.0(-M5), so I realized it's a regression. Is it going to be fixed for 2.10.0? If so, shouldn't this be reprioritized as critical to ensure that it's actually fixed before release? SI-5031 is currently marked as minor.
...
Thanks a lot! I think confusion has arisen because the _original_ bug report was about something else for 2.8.1 and 2.9.1, while Paul Phillips's comment switched the discussion to the regression. It might be worth preserving the original bug report.
But at least, the _regression_ won't be forgot.
