Ticket SI-2009 really contains two issues.   The basic idea is that message sends to an actor should never propagate an exception into the sender's thread.  Propagating errors (e.g. OOME) is ok.
