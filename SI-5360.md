...
Looking at scala.lang, it's true, there is no style defined for type _other_.  This can be easily fixed, for example by adding 
{{<style id="other" _name="Other" map-to="def:special-char"/>}}
to the <styles> sections.  _special-char_ isn't used for anything else, and in the default colourscheme, it's a plausible colour for annotations.
