...
But, if you think that memoization is useful, I could implement this functionality _separately_ from {{getOrElseUpdate}} and expose it on the {{TrieMap}} s.
...
Specifically, in the case of the {{TrieMap}}, we could additionally make the invocation atomic _and_ make sure that the mapping function is applied only once.
