...
{code}
The difference between the two errors is related to SI-9156. In the second case, the existence of the stable prefix `v` for the type means we don't need to capture an existential for `this` in an asSeenFrom, which ends up dealiasing `This` to `Foo`.
