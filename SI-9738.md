...
_> I think 6.26.4 case 3 applies. args are typed first with type params taken as constants, then as undefined._
...
In this second case, it appears as though the compiler either solves the type argument equation, with  result {{T == Long}}, _before_ overload resolution, or that the type bound {{T >: Long}} _is_ taken into account during overload resolution in a way that differs from the {{reduce(math.max)}} case.
