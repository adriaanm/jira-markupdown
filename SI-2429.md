...
{code}
The error message was similar to SI-2316, which I encountered recently porting Scalaz to 2.8. As such, I thought the recent fix for that bug may have triggered this one. As it turns out, they are unrelated.
...
Thanks for coming up with such a small example. This look similar to SI-2175 (an undeclared local variable is used in the pattern match translation). 
Note that there are two test cases with distinct errors in https://lampsvn.epfl.ch/trac/scala/attachment/ticket/2429/2429.scala that I extracted from Lift. The second one indeed looks like SI-2175. The first manifests a bit earlier, but may they may well be brethren.
