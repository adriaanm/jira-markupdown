...
[~extempore] It's a documentation error in that what is abstract and what is not doesn't match up with how things are actually delivered.  They _do_ override various methods to use head etc., but the documentation needs to spell out that it is your job to implement those efficiently AND then use those to implement the abstract methods.
