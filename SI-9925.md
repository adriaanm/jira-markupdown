...
Creating such tools _might_ push for switching to a slightly different EBNF syntax. (The current one doesn't have a token terminating each production, so it's not clear how to recognize the end of one production and the beginning of the next (at least without looking ahead for the "{{::=}}").))
