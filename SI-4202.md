...
That looks an awful lot like SI-2897, which is:
{code}
 def f1(t: String) = {
    trait T {
      def xs = Nil map (_ => t)
...
(In r24326) Closes SI-4202 and removes empty TreeHashMap (superseded by HashMap implementation). No review
...
> (In r24527) Closes SI-4202 again, closes SI-4363. if someone can enlighten me why we actually had that exclusion for companion objects in the first place I would be grateful.
