...
Thanks for reporting! I think this is because of the fix for SI-1786, and it looks related to SI-8198.
The fix for SI-1786 is useful, but it now also infers more precise bounds in expected types (it was mostly intended to have more precise types for "provided things").
...
[error]       this(new jfxsc.TableColumn.CellEditEvent(table, pos, eventType, newValue))
{noformat}
jpsacha: I don't think this SI-8198 related, but see SI-6169 for much further discussion.  It isn't a bug in the JavaFX API, because some of the bounds that it infers yield unspeakable existential types in Java, mostly on the *Builders.  With a fix for 6169, your interactions with JavaFX types should get smoother and less casty overall.
...well, except insofar as the non-F-bound case in SI-1786 interacts with the non-F-bound case of SI-6169 to cause your issues with non-F-bounded existentials, something similar would occur between SI-8198 and SI-6169, were the former fixed but not the latter.
I can confirm the first error is due to SI-6169. The other two are separate :(
note to self: can this subsume the fix for SI-1786
...
{code}
Looks related to https://issues.scala-lang.org/browse/SI-4728
