...
causes the following behaviour at runtime
...
Simpler case of this issue filed as SI-451 and fixed in r13906.

Lowered priority and postponed considering fix's complexity and considering SI-451 solves the most common case.
This is fixed in trunk!

{code}
scala> def printSize[T <% {def size: Int}](in: T) = println(in.size)
printSize: [T](in: T)(implicit view$$1: (T) => AnyRef{def size: Int})Unit

scala> 

scala> printSize(List(1, 2, 3))
3

scala> printSize("caw")
3

scala> printSize(Map(1 -> 2))
1

scala> printSize(Array(1, 2, 3))
3
{code}
This is not fixed.  The repl does not have the same execution semantics as a script.
{code}
$$ scala -i a.scala 
Loading a.scala...
printSize: [T](in: T)(implicit view$$1: (T) => AnyRef{def size: Int})Unit
