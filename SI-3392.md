...
In that case, see also SI-796.
ok, I think I fixed the delta wrt SI-796, which consists of setting the info for the variable that's bound to the pattern before typing the pattern

now my test crashes in the same way as SI-796, which is due to a missing variable declaration for Bind nodes
...
crash.  Closes SI-3392, no review.
...
That was useful enough to fix this one pretty easily; no effect on SI-796 though.
