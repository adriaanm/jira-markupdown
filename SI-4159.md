...
The first one should _never_ happen, because: 
...
Problem is that it doesn't crash with null for anything _except_ an explicit char[] in Java.

The problem in Scala is just the reverse: It always crashes with null _except_ a type different from Array[Char] is explicitly added to it.
